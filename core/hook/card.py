...
class Hook(BaseHook):
    ...
    def updateCard(self, action):
        ...
        if (
            "idAttachmentCover" in data["old"]
            and "idAttachmentCover" in data["card"]
        ):
        ...
        if (
            data["card"].get("idAttachmentCover")
            and "idAttachmentCover" in data["old"]
        ):
            return self.makeCoverToCard(action=action)
...
> {comment}"""
    ...
    def moveCardFromBoard(self, action):
        data = action["data"]
        context = {
            "board_target_name": data["boardTarget"].get("name")
            or data["board"].get("id"),
    ...
    def moveCardToBoard(self, action):
        data = action["data"]
        context = {
            "board_source_name": data["boardSource"].get("name")
            or data["boardSource"].get("id"),
