...
urlpatterns = [
    ...
    url(
        r"^callback/(?P<board_id>[0-9]+)/$",
        TrelloCallbacksView.as_view(),
        name="callback",
    ),
    ...
    url(
        r"^webhook/add/(?P<board_id>[0-9]+)/",
        WebhookCreateView.as_view(),
        name="webhook_create",
    ),
    url(
        r"^bridge/(?P<pk>[-\w]+)/$",
        BridgeDetailView.as_view(),
        name="bridge_detail",
    ),
    url(r"^bridges/$", BridgeListView.as_view(), name="bridge_list"),
    url(
        r"^bridge/add/(?P<board_id>[0-9]+)/(?P<webhook_id>[0-9]+)/",
        BridgeCreateView.as_view(),
        name="bridge_create",
    ),
]
